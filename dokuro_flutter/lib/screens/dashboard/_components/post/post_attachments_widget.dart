...
class PostAttachmentsWidget extends StatelessWidget {
  const PostAttachmentsWidget({Key? key, this.attachments}) : super(key: key);
  final PostAttachments? attachments;
  ...
  Widget _itemsRow(List<PostAttachment> items) {
    if (items.isEmpty) {
      return const SizedBox();
    }
